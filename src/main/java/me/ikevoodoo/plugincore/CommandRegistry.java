...
public class CommandRegistry {
    ...
    private record CommandData(Method method, ParameterType[] types, Object obj) {
        ...
        public boolean execute(CommandSender sender, String label, String[] args) {
            return getInstance().execute(method, types, obj, sender, label, args);
        }
    }
    ...
    private record BaseCommandData(BaseCommand cmd, List<SubCommandData> subCommands) {
        ...
        public boolean execute(CommandSender sender, String label, String[] args) {
            ...
            if(!getInstance().canUse(sender, subCommand.command().usableBy(), subCommand.command().permission())) {
                return true;
            }
    ...
    private void processBaseCommand(Object obj, Class<?> clazz) {
        ...
        for (Method method : clazz.getDeclaredMethods()) {
            if (method.isAnnotationPresent(SubCommand.class)) {
                ...
                SubCommandData data = new SubCommandData(subCommand, method, getTypes(method).toArray(new ParameterType[0]), obj);
    ...
    private void processMethods(Object obj, Class<?> clazz) {
        for(Method m : clazz.getDeclaredMethods()) {
            if(m.isAnnotationPresent(Command.class)) {
                ...
                CommandData data = new CommandData(m, getTypes(m).toArray(new ParameterType[0]), obj);
    ...
    private void registerBaseCommand(BaseCommandData data) {
        ...
        BukkitCommand command = new BukkitCommand(cmd.name()) {
            ...
            public boolean execute(@NotNull CommandSender sender, @NotNull String label, @NotNull String[] args) {
                if(!canUse(sender, cmd.usableBy(), null)) {
                    return true;
                }
    ...
    private void registerCommand(Command cmd, CommandData data) {
        ...
        BukkitCommand command = new BukkitCommand(cmd.name()) {
            @Override
            public boolean execute(@NotNull CommandSender sender, @NotNull String label, @NotNull String[] args) {
                if(!canUse(sender, cmd.usableBy(), cmd.permission())) {
                    return true;
                }
                return data.execute(sender, label, args);
            }
    ...
    private boolean execute(Method method, ParameterType[] types, Object obj, CommandSender sender, String label, String[] args) {
    ...
    private List<ParameterType> getTypes(Method m) {
    ...
    private boolean canUse(CommandSender sender, CommandUser user, String permission) {
        if(permission != null && !sender.hasPermission(permission)) {
            sender.sendMessage(ChatUtils.color("&cYou do not have permission to use this command!"));
            return false;
        }

        if(user != CommandUser.ALL)  {
            if(user == CommandUser.CONSOLE) {
            ...
            if(user == CommandUser.COMMAND_BLOCK) {
